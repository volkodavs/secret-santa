...
import com.sergeyvolkodav.secretsanta.repositories.FamilyRepository;

public class PartThree implements SecretSanta<Member> {

    // We want to choose year for calculation
    private int currentYear;
    private final FamilyRepository<Member> familyRepository;

    public PartThree(FamilyRepository<Member> familyRepository) {
    ...
    public Map<Member, Member> getPairs(Predicate<Member> filter) {
        List<Member> members = familyRepository.getMembers(filter);
        ...
        for (int i = 0; i < familySize; i++) {
            ...
            Member pairCandidate = findGiftCandidate(members, pairMemberIndex, member);
    ...
    private Member findGiftCandidate(List<Member> members, int pairMemberIndex, Member member) {
        ...
        while (nextElement < familySize) {
            ...
            if (!familyRepository.isFamilyMembers(member, pairCandidate)) {
                nextElement++;
                continue;
            }
